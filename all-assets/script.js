(function ($) {
	...
	function advanced_accordion_hashchange() {
		if ( location.hash && $(location.hash).length > 0 ) {
			...
			if ( element && element.length > 0 ) {
				location.href = '#';
				$('html, body').animate({
					scrollTop: ( element.parents('.elementor-toggle-item').offset().top - 50 ) + 'px'
				}, 500, function() {
					if ( ! element.parents('.elementor-tab-title').hasClass('elementor-active') ) {
    ...
    $(window).on('elementor/frontend/init', function () {
		...
		elementorFrontend.hooks.addAction('frontend/element_ready/eltemplate-faq.default', AdvancedAccordionHandler);
